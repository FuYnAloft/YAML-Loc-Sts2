...
# region 辅助函数
def extract_entries(data          , current_path                  = ())               :
...
def upper_snake_to_pascal(name     )       :
    if not name:
        return name
    return "".join(word.capitalize() for word in name.split("_"))


def yaml_to_entries(path      )               :
...
def entries_to_yaml(path      , entries                 )        :
...
def json_to_flat_entries(path      )                   :
    with open(path, 'r', encoding='utf-8-sig') as f:
        data = json.load(f)
    return [FlatEntry(k, v) for k, v in data.items()]


def flat_entries_to_json(path      , entries                     )        :
